...
Currently, the provider chain can [receive and verify evidence of equivocation](https://github.com/maany-xyz/ics/pull/1232), but it cannot slash the misbehaving validator.
...
Note that when a relayer or a user sends evidence through a [MsgSubmitConsumerDoubleVoting](https://github.com/maany-xyz/ics/pull/1232) message, the provider gets access to [DuplicateVoteEvidence](https://github.com/cometbft/cometbft/blob/v0.37.0/types/evidence.go#L35):
...
Similarly, when a relayer or a user sends evidence through a [MsgSubmitConsumerMisbehaviour](https://github.com/maany-xyz/ics/pull/826) message, the provider gets access to [Misbehaviour](https://github.com/cosmos/ibc-go/blob/v7.3.0/proto/ibc/lightclients/tendermint/v1/tendermint.proto#L79) that we cannot use to extract the infraction height, power, or the time on the provider chain.
...
- [ADR 005: Cryptographic verification of equivocation evidence](https://github.com/maany-xyz/ics/blob/main/docs/docs/adrs/adr-005-cryptographic-equivocation-verification.md)
- [EPIC tracking cryptographic equivocation feature](https://github.com/maany-xyz/ics/issues/732)
