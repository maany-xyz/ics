...
- If `SlashPackets` or `VSCMaturedPackets` are actually throttled/queued on the provider, state can grow and potentially lead to a DoS attack.
  ...
  See [#594](https://github.com/maany-xyz/ics/issues/594).
...
For the implementation of this design, see [throttle_retry.go](https://github.com/maany-xyz/ics/blob/fec3eccad59416cbdb6844e279f59e3f81242888/x/ccv/consumer/keeper/throttle_retry.go).
...
The consumer ccv module previously queued "pending packets" to be sent in each `EndBlock` in [SendPackets](https://github.com/maany-xyz/ics/blob/3bc4e7135066d848aac60b0787364c07157fd36d/x/ccv/consumer/keeper/relay.go#L178).
...
See older version of [AppendPendingPacket](https://github.com/maany-xyz/ics/blob/05c2dae7c6372b1252b9e97215d07c6aa7618f33/x/ccv/consumer/keeper/keeper.go#L606).
...
This feature will implement consumer changes in [#1024](https://github.com/maany-xyz/ics/pull/1024).
...
In other words, the consumer changes in [#1024](https://github.com/maany-xyz/ics/pull/1024) are compatible with the current ("v1") provider implementation of throttling that's running on the Cosmos Hub as of July 2023.

Once all consumers have deployed the changes in #1024, the provider changes from [#1321](https://github.com/maany-xyz/ics/pull/1321) can be deployed to production, fully enabling v2 throttling.
...
- Some issues like [#1001](https://github.com/maany-xyz/ics/issues/1001) will now be handled implicitly by the improved throttling mechanism.
...
- [EPIC](https://github.com/maany-xyz/ics/issues/713) tracking the changes proposed by this ADR
- [ADR 002: Jail Throttling](./adr-002-throttle.md)
- [#594](https://github.com/maany-xyz/ics/issues/594)
